...
TEST_F(SmpBrProcessPairingCommandTest, test_ctkd_not_bonded_br_edr) {
  // Not bonded
  p_dev_rec_->bond_type = tBTM_SEC_DEV_REC::BOND_TYPE_UNKNOWN;
  p_dev_rec_->link_key_type = BTM_LKEY_TYPE_UNAUTH_COMB_P_256;
  CallProcessPairingCommand();
...
TEST_F(SmpBrProcessPairingCommandTest, test_ctkd_temp_paired) {
  // Temporarily paired
  p_dev_rec_->bond_type = tBTM_SEC_DEV_REC::BOND_TYPE_TEMPORARY;
  p_dev_rec_->set_link_key_known();
  p_dev_rec_->link_key_type = BTM_LKEY_TYPE_UNAUTH_COMB_P_256;
  p_dev_rec_->sec_flags |= BTM_SEC_ENCRYPTED;
  CallProcessPairingCommand();
  EXPECT_EQ(cb_.status, SMP_XTRANS_DERIVE_NOT_ALLOW);
}
...
TEST_F(SmpBrProcessPairingCommandTest, test_ctkd_no_sc) {
  ...
  p_dev_rec_->bond_type = tBTM_SEC_DEV_REC::BOND_TYPE_PERSISTENT;
  p_dev_rec_->set_link_key_known();
  p_dev_rec_->link_key_type = BTM_LKEY_TYPE_COMBINATION;
  p_dev_rec_->sec_flags |= BTM_SEC_ENCRYPTED;
  CallProcessPairingCommand();
  EXPECT_EQ(cb_.status, SMP_XTRANS_DERIVE_NOT_ALLOW);
}
...
TEST_F(SmpBrProcessPairingCommandTest, test_ctkd_device_not_encrypted) {
  ...
  p_dev_rec_->set_link_key_known();
  p_dev_rec_->link_key_type = BTM_LKEY_TYPE_UNAUTH_COMB_P_256;
  p_dev_rec_->sec_flags &= ~BTM_SEC_ENCRYPTED;  // Not encrypted
...
TEST_F(SmpBrProcessPairingCommandTest, test_ctkd_already_bonded_le_higher_security) {
  ...
  p_dev_rec_->sec_flags |= BTM_SEC_LE_LINK_KEY_AUTHED;
  // BR link key is NOT authed
  p_dev_rec_->sec_flags &= ~BTM_SEC_LINK_KEY_AUTHED;

  // Setup BLE keys so is_bonded(LE) logic passes
  p_dev_rec_->set_le_link_key_known();
  p_dev_rec_->ble.key_type = BTM_LE_KEY_PENC;
...
TEST_F(SmpBrProcessPairingCommandTest, test_success_as_peripheral) {
  ...
  EXPECT_FALSE(p_dev_rec_->new_encryption_key_is_p256);
}
