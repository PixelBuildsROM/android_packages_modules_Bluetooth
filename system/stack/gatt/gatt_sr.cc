...
void gatts_chk_pending_ind(tGATT_TCB& tcb) {
  ...
  if (status != GATT_SUCCESS && status != GATT_PENDING) {
    LOG(WARNING) << "Unable to send GATT server handle value conn_id:" << buf.conn_id;
  }
}
...
void gatts_chk_pending_notif(tGATT_TCB& tcb) {
  ...
  while (count > 0) {
    ...
    if (std::holds_alternative<tGATT_VALUE>(buf)) {
      ...
      if (status != GATT_SUCCESS && status != GATT_PENDING) {
        LOG(WARNING) << "Unable to send GATT server handle value conn_id:" << notif.conn_id << " status: " << static_cast<int>(status);
      }
      ...
      if (status != GATT_SUCCESS && status != GATT_PENDING) {
        LOG(WARNING) << "Unable to send GATT server handle multiple value notif conn_id:" << multi_notif.front().conn_id << " status: " << static_cast<int>(status);
      }
      if (status != GATT_PENDING) {
        tGATTS_DATA gatts_data;
